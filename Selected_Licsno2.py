...
def selected_licsno_code2(spark):
    ...
    for indexHIST in list_HIST:
        for indexCRCAMF in list_CRCAMF:
            df_CRCAMF = df_CRCAMF.repartition(indexCRCAMF, "CARNM_M").join(
                df_SSHSCHISTORY.where(df_SSHSCHISTORY[indexHIST] != '')
    ...
    df_CRCAMF = df_CRCAMF.repartition("EGNO_6", "CARNM_M").join(
        df_SSHSCHISTORY.filter(df_SSHSCHISTORY['ENGINENO_6'] != '')
        .select("ENGINENO_6", "GRPNM", "is_scrapped").dropDuplicates(["ENGINENO_6", "GRPNM"]).repartition("ENGINENO_6",
                                                                                                          "GRPNM"),
    ...
    df_CRCAMF = df_CRCAMF.repartition("BDNO_6", "CARNM_M").join(
        df_SSHSCHISTORY.filter(df_SSHSCHISTORY['BODYNO_6'] != '').select("BODYNO_6", "GRPNM", "is_scrapped")
