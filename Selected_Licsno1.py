...
from Selected_Licsno1_1 import selected_licsno_code1_1

def selected_licsno_code1(spark, Log_File):
    ...
    #df_CRCAMF.write.option('header', 'true').csv(Temp_Path + "df_CRCAMF_remove_LICSNO.csv")
    ...
    #df_SSHSCHISTORY.write.option('header', 'true').csv(Temp_Path + "df_SSHSCHISTORY_clean.csv")
    ...
    # 9碼以下不清 後九碼相同者視為已報廢
    #remove_LICSNO_list = df_web_query.filter(expr("length(EGNO)>8")).rdd.map(lambda row: str(row.EGNO)).collect()
    ...
    #df_SSHSCHISTORY = df_SSHSCHISTORY.where(~(col("ENGINENO_temp").isin(remove_LICSNO_list)))
    ...
    #df_SSHSCHISTORY.write.option('header', 'true').csv(Temp_Path + "df_SSHSCHISTORY.csv")
    #df_CRCAMF.write.option('header', 'true').csv(Temp_Path + "df_CRCAMF.csv")
    #df_CRCAMF_web_query20150708.write.option('header', 'true').csv(Temp_Path + "df_CRCAMF_web_query20150708.csv")
    #selected_licsno_code1_1(spark)
