...
def LRDT_calculator_dask(df_input, groupby_column, cal_column_name):
    ...
    df_result.columns = [cal_column_name+'_min',cal_column_name+'_max',cal_column_name+'_count',cal_column_name+'_range']
    return df_result
...
def dataframe_quick_look(df_input):
    ...
    df_feature_desc[u'NA_Rate'] = df_feature_desc[u'columns'].map(lambda x:         float(df_input[df_input[x].isnull()].shape[0] )/float (df_input.shape[0])  )
...
def getSRMINVO_query_data():
    ...
    df_temp2 = pd.read_sql(u"""select VIN, DLRCD, BRNHCD, WORKNO from cdp.SRMSLPH where VIN!='' and VIN IS NOT NULL and CMPTDT IS NOT NULL""", conn) 
    df_temp3 = pd.read_sql(u"""select INVSTS, DETRMK, INVONO, INVODT, DLRCD, BRNHCD, INVTXCD, TOTAMT, INSURCD, IRNAMT, WSHAMT from cdp.SRMINVO""", conn)
    ...
    df_SRMINVO = pd.DataFrame(df_SRMINVO, columns=['VIN','INVODT', 'DLRCD', 'BRNHCD', 'INVONO', 'INVTXCD', 'TOTAMT', 'INSURCD', 'IRNAMT', 'WSHAMT'])

    df_temp1 = pd.read_sql(u"""select DLRCD, BRNHCD, WORKNO, INVONO from cdp.SRHIVSLP""", conn)
    df_temp2 = pd.read_sql(u"""select VIN, DLRCD, BRNHCD, WORKNO from cdp.SRHSLPH where VIN!='' and VIN IS NOT NULL and CMPTDT IS NOT NULL""", conn) 
    df_temp3 = pd.read_sql(u"""select INVSTS, DETRMK, INVONO, INVODT, DLRCD, BRNHCD, INVTXCD, TOTAMT, INSURCD, IRNAMT, WSHAMT from cdp.SRHINVO""", conn)    
    ...
    df_temp2 = pd.read_sql(u"""select VIN, DLRCD, BRNHCD, WORKNO from cdp.SRHSLPH15 where VIN!='' and VIN IS NOT NULL and CMPTDT IS NOT NULL""", conn) 
    df_temp3 = pd.read_sql(u"""select INVSTS, DETRMK, INVONO, INVODT, DLRCD, BRNHCD, INVTXCD, TOTAMT, INSURCD, IRNAMT, WSHAMT from cdp.SRHINVO15""", conn)        
    df_temp3 = df_temp3[~(df_temp3['INVSTS']=='B')]
    df_temp3 = df_temp3[~(df_temp3['INVSTS']=='C')]
    df_temp3 = df_temp3[~(df_temp3['INVSTS']=='D')]
    df_temp3 = df_temp3[~(df_temp3['INVSTS']=='E')]
...
def DTtime_cleaner(Input_Series, data_type=0):
        ...
        if data_type==1:
            ...
            temp.loc[(temp[u'DT_fix']>15000) | (temp[u'DT_fix']<0), u'DT_fix'] = df_LICSNO0717_Features.loc            [(temp[u'DT_fix']>15000) | (temp[u'DT_fix']<0), u'CARAGE']*365
        elif data_type==2:
            temp[u'DT_fix'] = pd.to_datetime(Input_Series, errors = 'coerce')
            temp[u'DT_fix'].fillna( pd.Timestamp('1900-01-01'), inplace=True)
            temp[u'DT_fix'] = (pd.Timestamp(END_DATE) - temp[u'DT_fix']).map(lambda x: x.days) 
            temp.loc[(temp[u'DT_fix']>15000), u'DT_fix'] = df_LICSNO0717_Features.            loc[(temp[u'DT_fix']>15000) , u'CARAGE']*365       
        else:
            temp[u'DT'] = Input_Series.map(lambda x: x if (isinstance(x, float)or x is None ) else x.replace(' ', ''))
            temp[u'DT_fix'] = pd.to_datetime(temp[u'DT'], format='%Y%m%d', errors = 'coerce')
            temp[u'DT_fix'].fillna( pd.Timestamp('1900-01-01'), inplace=True)
...
if __name__ == '__main__':
  ...
  write_Log(Log_File,"09. %s | Compare with scrapped data from web query and df_SSHSCHISTORY......"%str(datetime.datetime.now()))
  df_SSHSCHISTORY = Original_df_SSHSCHISTORY.copy() ##SQL 執行一次之後，儲存於 Original_df_XXX，要用時直接copy出來，節省執行時間
  ...
  df_SSHSCHISTORY = df_SSHSCHISTORY[ df_SSHSCHISTORY['MODDT_fix'] < datetime.datetime(2015,7,8,0,0,0,0)] #監理站資料，移除2015/07/08之後報廢的資料
  ...
  merged = None
  merged = df_CRCAMF[['EGNO_6', 'CARNM_M']].merge(df_SSHSCHISTORY[['ENGINENO_6','GRPNM','is_scrapped']]                                [df_SSHSCHISTORY['ENGINENO_6']!=''].drop_duplicates(['ENGINENO_6','GRPNM'])                                    , how='left', left_on=['EGNO_6','CARNM_M'], right_on=['ENGINENO_6','GRPNM'])
  merged['is_scrapped'] =  merged['is_scrapped'].fillna('0')
  df_CRCAMF['EGNO_6_ENGINENO_6'] = merged['is_scrapped'].reset_index(drop=True)

  merged = None
  merged = df_CRCAMF[['BDNO_6', 'CARNM_M']].merge(df_SSHSCHISTORY[['BODYNO_6','GRPNM','is_scrapped']]                                [df_SSHSCHISTORY['BODYNO_6']!=''].drop_duplicates(['BODYNO_6','GRPNM'])                                    , how='left', left_on=['BDNO_6','CARNM_M'], right_on=['BODYNO_6','GRPNM'])
  merged['is_scrapped'] =  merged['is_scrapped'].fillna('0')
  ...
  ##重複者一併刪除 數量約在3000以下
  #任一條件皆刪除
  df_CRCAMF = df_CRCAMF[(df_CRCAMF['BDNO_ENGINENO']=='0')&                (df_CRCAMF['EGNO_ENGINENO']=='0')&                (df_CRCAMF['VIN_ENGINENO']=='0')&                (df_CRCAMF['BDNO_BODYNO']=='0')&                (df_CRCAMF['EGNO_BODYNO']=='0')&                (df_CRCAMF['VIN_BODYNO']=='0')&                (df_CRCAMF['BDNO_EGNOM']=='0')&                (df_CRCAMF['EGNO_EGNOM']=='0')&                (df_CRCAMF['VIN_EGNOM']=='0')&                (df_CRCAMF['BDNO_BDNOM']=='0')&                (df_CRCAMF['EGNO_BDNOM']=='0')&                (df_CRCAMF['VIN_BDNOM']=='0')&                (df_CRCAMF['EGNO_6_ENGINENO_6']=='0')&                (df_CRCAMF['BDNO_6_BODYNO_6']=='0')][['LICSNO', u'CARNM', 'CARNM_M', u'CARMDL','BDNO','EGNO', 'VIN','BDNO_6','EGNO_6', 'UCDELIVIDT']]
  ...
  df_CRCAMF_NOTused = df_CRCAMF_NOTused[~df_CRCAMF_NOTused['VIN'].isin(    df_CRCAMF_used['VIN'][df_CRCAMF_used['VIN'].map(lambda x: len(x))>5].tolist())]
  df_CRCAMF_NOTused = df_CRCAMF_NOTused[~df_CRCAMF_NOTused['EGNO'].isin(    df_CRCAMF_used['EGNO'][df_CRCAMF_used['EGNO'].map(lambda x: len(x))>5].tolist())]
  df_CRCAMF_NOTused = df_CRCAMF_NOTused[~df_CRCAMF_NOTused['BDNO'].isin(    df_CRCAMF_used['BDNO'][df_CRCAMF_used['BDNO'].map(lambda x: len(x))>5].tolist())]
  ...
  df_SSHUCHISTORY = df_SSHUCHISTORY[(df_SSHUCHISTORY[u'ISSUE_fix'] <= datetime.datetime(today.year - Candidate_Car_age,today.month,1,0,0,0,0)) &                      (df_SSHUCHISTORY[u'ISSUE_fix'] >= datetime.datetime(1988,1,1,0,0,0,0)) ]
  ...
  df_SSHUCHISTORY[u'EGNOM'] = df_SSHUCHISTORY.apply(lambda x: x['EGNOM'] if x['EGNOM'] not in x['ENGINENO'] else '', axis=1)
  df_SSHUCHISTORY[u'BDNOM'] = df_SSHUCHISTORY.apply(lambda x: x['BDNOM'] if x['BDNOM'] not in x['BODYNO'] else '', axis=1)
  ...
  #         print (indexHIST, indexCRCAMF)
          merged = None
          merged = df_CRCAMF_NOTused[['BDNO', 'EGNO', 'VIN','CARNM_M']].merge(df_SSHUCHISTORY[df_SSHUCHISTORY[indexHIST]!='']                [[indexHIST,'GRPNM','is_scrapped']].drop_duplicates(indexHIST), how='left', left_on=[indexCRCAMF,'CARNM_M'] , right_on=[indexHIST,'GRPNM'])
          merged['is_scrapped'] =  merged['is_scrapped'].fillna('0')
  ...
  #只選六碼時 限定 BD VS BD  , EG VS EG 避免 因為碼數較小誤判
  merged = None
  merged = df_CRCAMF_NOTused[['EGNO_6', 'CARNM_M']].merge(df_SSHUCHISTORY[['ENGINENO_6','GRPNM','is_scrapped']]                            [df_SSHUCHISTORY['ENGINENO_6']!=''].drop_duplicates(['ENGINENO_6','GRPNM'])                                    , how='left', left_on=['EGNO_6','CARNM_M'], right_on=['ENGINENO_6','GRPNM'])
  merged['is_scrapped'] =  merged['is_scrapped'].fillna('0')
  df_CRCAMF_NOTused['EGNO_6_ENGINENO_6'] = merged['is_scrapped'].reset_index(drop=True)

  merged = None
  merged = df_CRCAMF_NOTused[['BDNO_6', 'CARNM_M']].merge(df_SSHUCHISTORY[['BODYNO_6','GRPNM','is_scrapped']]                            [df_SSHUCHISTORY['BODYNO_6']!=''].drop_duplicates(['BODYNO_6','GRPNM'])                                    , how='left', left_on=['BDNO_6','CARNM_M'], right_on=['BODYNO_6','GRPNM'])
  merged['is_scrapped'] =  merged['is_scrapped'].fillna('0')
  df_CRCAMF_NOTused['BDNO_6_BODYNO_6'] = merged['is_scrapped'].reset_index(drop=True)
  del df_SSHUCHISTORY
  df_CRCAMF_NOTused = df_CRCAMF_NOTused[(df_CRCAMF_NOTused['BDNO_ENGINENO']=='0')&                (df_CRCAMF_NOTused['EGNO_ENGINENO']=='0')&                (df_CRCAMF_NOTused['VIN_ENGINENO']=='0')&                (df_CRCAMF_NOTused['BDNO_BODYNO']=='0')&                (df_CRCAMF_NOTused['EGNO_BODYNO']=='0')&                (df_CRCAMF_NOTused['VIN_BODYNO']=='0')&                (df_CRCAMF_NOTused['BDNO_EGNOM']=='0')&                (df_CRCAMF_NOTused['EGNO_EGNOM']=='0')&                (df_CRCAMF_NOTused['VIN_EGNOM']=='0')&                (df_CRCAMF_NOTused['BDNO_BDNOM']=='0')&                (df_CRCAMF_NOTused['EGNO_BDNOM']=='0')&                (df_CRCAMF_NOTused['VIN_BDNOM']=='0')&                 (df_CRCAMF_NOTused['EGNO_6_ENGINENO_6']=='0')&                (df_CRCAMF_NOTused['BDNO_6_BODYNO_6']=='0')][['LICSNO', u'CARNM', u'CARMDL','BDNO','EGNO', 'VIN','UCDELIVIDT']]
  #移除相關BDNO_ENGINENO 避免後續過戶資料重複

  df_CRCAMF = df_CRCAMF_NOTused.append(df_CRCAMF_used, ignore_index=True)
  #找出 最近三年內有回廠的15年以上車
  write_Log(Log_File,"13. %s | Look out the repaired cars over 15 years old in the past 3 years......"%str(datetime.datetime.now()))
  conn = pyodbc.connect(DSN="Simba Spark ODBC Driver",autocommit=True, unicode_results=True)
  ...
  #修正 NaT 為 isnull()
  df_CRCAMF_3year.loc[df_CRCAMF_3year[u'LSKIDT_stamp'].isnull(), u'LSKIDT_stamp']         = df_CRCAMF_3year.loc[df_CRCAMF_3year[u'LSKIDT_stamp'].isnull(), u'STRDT']

  # print u'1. 暫用15年以上車輛' 
  df_CRCAMF_3year = df_CRCAMF_3year[(df_CRCAMF_3year[u'LSKIDT_stamp'] <= datetime.datetime(today.year - Candidate_Car_age,today.month,1,0,0,0,0)) &                      (df_CRCAMF_3year[u'LSKIDT_stamp'] >= datetime.datetime(1988,1,1,0,0,0,0)) ]
  write_Log(Log_File,"ok\n")

  write_Log(Log_File,"14. %s | Remove unused data......"%str(datetime.datetime.now()))
  # print u'2. 去除買不到一年的中古車' #自2018/01/01 起算 回推一年
  df_CRCAMF_3year = df_CRCAMF_3year[(df_CRCAMF_3year[u'UCDELIVIDT'] <= datetime.datetime(today.year -1,today.month,1,0,0,0,0))|                       df_CRCAMF_3year[u'UCDELIVIDT'].isnull()]
  ...
  df_CRAURF_select_LICSNO['LICSNO'] = df_CRAURF_select_LICSNO['LICSNO'].map(lambda x: x if (isinstance(x, float) or x is None ) else x.strip()) 
  ...
  df_combined = df_CRCAMF[['LICSNO','qualified']].append(df_CRAURF_select_LICSNO[['LICSNO','qualified']], ignore_index=True)
  ...
  conn = pyodbc.connect(DSN="Simba Spark ODBC Driver",autocommit=True, unicode_results=True)
  df_PSLTAXORDMF = pd.read_sql(u"SELECT * FROM cdp.PSLTAXORDMF WHERE OBRAND like '%TOYOTA%' or OBRAND like '%LEXUS%' or OBRAND like '%AMCTOYOTA%' or OBRAND like '%TOYOYA%' or OBRAND like '%豐田%' or OBRAND like '%YOYOTA%' or OBRAND like '%國%' or OBRAND like '%T0YOTA%' or OBRAND like '%TOYATA%'", conn)
  conn.close()
  ...
  df_combined = df_combined.merge(df_PSLTAXORDMF[['OLICSNO', 'PSLTAX']], how='left', left_on='LICSNO', right_on = 'OLICSNO')
  ...
  df_scrapped_20150708 = pd.read_csv(Temp_Path+"df_CRCAMF_scrapped_after20150708.csv", sep=',', encoding = 'utf-8' )
  df_scrapped_20150708['LICSNO'] = df_scrapped_20150708['LICSNO'].map(lambda x: x if (isinstance(x, float) or x is None ) else x.strip()) 
  df_scrapped_20150708['scrapped_150708']='1'
  df_combined = df_combined.merge(df_scrapped_20150708[['LICSNO', 'scrapped_150708']], how='left', left_on='LICSNO', right_on = 'LICSNO')
  ...
  df_combined = df_combined[~((df_combined[u'EGNO'].isnull())&(df_combined[u'BDNO'].isnull())&(df_combined[u'FRAN'].isnull()))]
  df_combined = df_combined.reset_index(drop=True)
  ...
  df_combined['CUSTID1'] = df_combined['CUSTID1'].map(lambda x: x if (isinstance(x, float)or x is None ) else x.strip())
  df_combined['CUSTID2'] = df_combined['CUSTID2'].map(lambda x: x if (isinstance(x, float)or x is None ) else x.strip())
  ...
  df_combined = df_combined[(df_combined['qualified']=='0' ) | ((df_temp['lenCUSTID2']==10) & (df_combined['qualified']=='1' ) )]
  ...
  for index in obj_list:
      df_combined[index] = df_combined[index].map(lambda x: x if (isinstance(x, float)or x is None or x is not datetime.datetime) else x.strip())    

  df_combined = df_combined[~((df_combined.qualified=='0') & (df_combined.scrapped_150708=='1')& (df_combined.PSLTAX=='1'))]
  write_Log(Log_File,"ok\n")
  ...
  write_Log(Log_File,"17. %s | Features construction from cdp.SRMSLPH and cdp.ISMAIN......"%str(datetime.datetime.now()))
  result_queue = []
  return_index=[None] * 6

  SQL1 = (u"""SELECT VIN, WSLPDT, RTPTDT, STRWKDT, FINDT, AGVCARDT, CMPTDT FROM cdp.SRMSLPH WHERE CMPTDT IS NOT NULL""")
  SQL2 = (u"""SELECT  VIN, WSLPDT, RTPTDT, STRWKDT,FINDT,AGVCARDT, CMPTDT  FROM cdp.SRHSLPH WHERE CMPTDT IS NOT NULL""")
  SQL3 = (u"""SELECT  VIN, WSLPDT, RTPTDT, STRWKDT,FINDT,AGVCARDT, CMPTDT  FROM cdp.SRHSLPH15 WHERE CMPTDT IS NOT NULL""")
  ...
  df_ISMAIN_all = None
  write_Log(Log_File,"ok\n")
  write_Log(Log_File,"18. %s | Features construction from cdp.CRAURF and cdp.CRCAMF......"%str(datetime.datetime.now()))
  ...
  df_CRAURF1.loc[df_CRAURF1[u'LSKIDT_stamp']=='NaT', u'LSKIDT_stamp'] = df_CRAURF1.loc[df_CRAURF1[u'LSKIDT_stamp']=='NaT', u'STRDT']
  # STRDT: 建檔日期，from CRCAMF

  df_CRAURF2[u'LSKIDT_stamp'] = df_CRAURF2[[u'LSKIDT_fix', u'SEDLDT_fix']].min(axis=1)
  df_CRAURF2.loc[df_CRAURF2[u'LSKIDT_stamp']=='NaT', u'LSKIDT_stamp'] = df_CRAURF2.loc[df_CRAURF2[u'LSKIDT_stamp']=='NaT', u'STRDT']
  #最近新買的車移除
  ...
  df_CRAURF_join12 = df_CRAURF1[['LICSNO', 'CUSTID1']].merge(df_CRAURF2[['LICSNO', 'CUSTID2']], how='left', on='LICSNO')
  #確認df_CRAURF1 df_CRAURF2都沒有重複車牌

  #計算 領照人名下領照人車輛數 領照人名下使用人車輛數 使用人名下領照人車輛數  使用人名下使用人車輛數 
  df_CRAURF_join12 = df_CRAURF_join12.merge(result_CARCNT_CUSTID1_nunique, how='left', left_on='CUSTID1',  right_on='CUSTID1', suffixes=('', '_y'))
  df_CRAURF_join12 = df_CRAURF_join12.rename(index=str, columns={u'LICSNO_y':u'carCNT11'})

  df_CRAURF_join12 = df_CRAURF_join12.merge(result_CARCNT_CUSTID1_nunique, how='left', left_on='CUSTID2',  right_on='CUSTID1', suffixes=('', '_y'))
  ...
  df_CRAURF_join12 = df_CRAURF_join12.merge(result_CARCNT_CUSTID2_nunique, how='left', left_on='CUSTID1',  right_on='CUSTID2', suffixes=('', '_y'))
  df_CRAURF_join12 = df_CRAURF_join12.rename(index=str, columns={u'LICSNO_y':u'carCNT12'})

  df_CRAURF_join12 = df_CRAURF_join12.merge(result_CARCNT_CUSTID2_nunique, how='left', left_on='CUSTID2',  right_on='CUSTID2', suffixes=('', '_y'))
  ...
  df_CRAURF2 = None
  write_Log(Log_File,"ok\n")
  write_Log(Log_File,"19. %s | Features construction from cdp.SRMSLPH, cdp.SRHSLPH, and SRHSLPH15......"%str(datetime.datetime.now()))       

  # sr定保 維修 次數計算
  conn = pyodbc.connect(DSN="Simba Spark ODBC Driver",autocommit=True, unicode_results=True)
  df_SRMSLPH = pd.read_sql(u"""SELECT VIN, WORKNO, WSLPDT, WKCTCD, PAYCD, INJ, CMPTDT FROM cdp.SRMSLPH WHERE CMPTDT IS NOT NULL""", conn)
  df_SRHSLPH = pd.read_sql(u"""SELECT VIN, WORKNO, WSLPDT, WKCTCD, PAYCD, INJ, CMPTDT FROM cdp.SRHSLPH WHERE CMPTDT IS NOT NULL""", conn)
  df_SRHSLPH15 = pd.read_sql(u"""SELECT  VIN, WORKNO, WSLPDT, WKCTCD, PAYCD, INJ, CMPTDT FROM cdp.SRHSLPH15 WHERE CMPTDT IS NOT NULL""", conn)
  conn.close()
  ...
  result_SR_WKCTCD.columns = ['WKCTCD_star_counts','WKCTCD_1_counts','WKCTCD_2_counts','WKCTCD_3_counts',                            'WKCTCD_B_counts','WKCTCD_P_counts','WKCTCD_T_counts',                           'PAYCD_counts','PAYCD_A_counts','PAYCD_B_counts','PAYCD_C_counts',                           'INJ_A_counts','INJ_B_counts','INJ_C_counts','INJ_D_counts',                           'WKCTCD_star_counts_3year','WKCTCD_1_counts_3year','WKCTCD_2_counts_3year','WKCTCD_3_counts_3year',                            'WKCTCD_B_counts_3year','WKCTCD_P_counts_3year','WKCTCD_T_counts_3year',                           'PAYCD_counts_3year','PAYCD_A_counts_3year','PAYCD_B_counts_3year','PAYCD_C_counts_3year',                           'INJ_A_counts_3year','INJ_B_counts_3year','INJ_C_counts_3year','INJ_D_counts_3year']
  ...
  #             print (index)
              df_input[df_input.columns[index]] = df_input[df_input.columns[index]].map(lambda x: str(x).strip() if x != None else None )          
  #     print ('done')
      return df_input
  ...
  result_DLRCD_BR_maxoccu = df_SRLFOWM[['DLRCD_BR','VIN']].groupby('VIN').agg(lambda x: x.value_counts(dropna=False).index[0])
  ...
  result_SRVNO_maxoccu = df_SRLFOWM[['SRVNO','VIN']].groupby('VIN').agg(lambda x: x.value_counts(dropna=False).index[0])
  ...
  result_INVODT_max['INVODT'] = (pd.Timestamp(END_DATE) - result_INVODT_max['INVODT']).map(lambda x: x if pd.isnull(x) else x.days) 
  result_INVODT_min['INVODT'] = (pd.Timestamp(END_DATE) - result_INVODT_min['INVODT']).map(lambda x: x if pd.isnull(x) else x.days) 
  ...
  result_SRMINVO_DLRCD_maxoccu = df_allSRMINVO[['DLRCD_BRNHCD','VIN']].groupby('VIN').agg(lambda x: x.value_counts(dropna=False).index[0])#

  # INVONO  *免費  #出保固 次數
  result_SRMINVO_INVONO_freecount = df_allSRMINVO[df_allSRMINVO['INVONO'].str[0]=='*'][['INVONO','VIN']].groupby('VIN').agg('count')#
  result_SRMINVO_INVONO_quarcount = df_allSRMINVO[df_allSRMINVO['INVONO'].str[0]=='#'][['INVONO','VIN']].groupby('VIN').agg('count')#

  #INVTXCD 主要發票聯式
  result_SRMINVO_INVTXCD_maxoccu = df_allSRMINVO[['INVTXCD','VIN']].groupby('VIN').agg(lambda x: x.value_counts(dropna=False).index[0])#
  ...
  #INSURCD 保險公司出險次數
  result_SRMINVO_INSURCD_count = df_allSRMINVO[df_allSRMINVO['INSURCD']!=''][['INSURCD','VIN']].groupby('VIN').agg('count')#

  #IRNAMT 板金次數
  result_SRMINVO_IRNAMT_count = df_allSRMINVO[df_allSRMINVO['IRNAMT']>0][['IRNAMT','VIN']].groupby('VIN').agg('count')#

  #WSHAMT 噴漆次數
  result_SRMINVO_WSHAMT_count = df_allSRMINVO[df_allSRMINVO['WSHAMT']>0][['WSHAMT','VIN']].groupby('VIN').agg('count')#
  # df_allSRMINVO=None
  ...
  result_PPAID_diff_DLRCD_BRNHCD_count = df_SRPPDPMF[['DLRCD_BRNHCD','VIN']].groupby('VIN').agg(lambda x: x.nunique())
  ...
  result_PPAIDuse_diff_DLRCD_BRNHCD_count = df_SRPPDCMF[['DLRCD_BRNHCD','VIN']].groupby('VIN').agg(lambda x: x.nunique())
  ...
  #買預付與使用不同地點的次數
  result_PPAID_diff_sa_use_times = df_SRPPDCMF[['DLRCD','VIN']][df_SRPPDCMF['DLRCD_BRNHCD']!=df_SRPPDCMF['SADLRCD_BRNHCD']].groupby('VIN').agg('count')
  ...
  for index in range(0,len(df_CSMCFORM.columns)):
      if df_CSMCFORM[df_CSMCFORM.columns[index]].dtypes=='object':
          ...
          print (index)
          df_CSMCFORM[df_CSMCFORM.columns[index]] = df_CSMCFORM[df_CSMCFORM.columns[index]].map(lambda x: x.strip() if x != None else None )
  print ('done')
  ...
  result_CONTTYPE_maxoccu = df_CSMCFORM[['CONTTYPE','LICSNO']].groupby('LICSNO').agg(lambda x: x.value_counts(dropna=False).index[0])
  ...
  result_SOURCETP_maxoccu = df_CSMCFORM[['SOURCETP','LICSNO']].groupby('LICSNO').agg(lambda x: x.value_counts(dropna=False).index[0])
  #CASETP R Q G C 次數   CSMCFORM_CASETP_Rcount  CASETP_CASETP_Qcount  CSMCFORM_CASETP_Gcount  CSMCFORM_CASETP_Ccount
  result_CASETP_Rcount = df_CSMCFORM[df_CSMCFORM['CASETP']=='R'][['CASEID','LICSNO']].groupby('LICSNO').agg('count')
  result_CASETP_Qcount = df_CSMCFORM[df_CSMCFORM['CASETP']=='Q'][['CASEID','LICSNO']].groupby('LICSNO').agg('count')
  result_CASETP_Gcount = df_CSMCFORM[df_CSMCFORM['CASETP']=='G'][['CASEID','LICSNO']].groupby('LICSNO').agg('count')
  result_CASETP_Ccount = df_CSMCFORM[df_CSMCFORM['CASETP']=='C'][['CASEID','LICSNO']].groupby('LICSNO').agg('count')
  ...
  result_BRNHTP_if0 = df_CSMCFORM[df_CSMCFORM['BRNHTP']=='0'][['CASEID','LICSNO']].groupby('LICSNO').agg(lambda x: 1.0 if x.nunique()>0 else 0.0)
  result_BRNHTP_if1 = df_CSMCFORM[df_CSMCFORM['BRNHTP']=='*'][['CASEID','LICSNO']].groupby('LICSNO').agg(lambda x: 1.0 if x.nunique()>0 else 0.0)
  ...
  result_CLOSESTAT_maxoccu = df_CSMCFORM[['CLOSESTAT','LICSNO']].groupby('LICSNO').agg(lambda x: x.value_counts(dropna=False).index[0])
  ...
  df_Feature_SRLFOWM_0717 = pd.read_csv(Temp_Path+"Feature_SRLFOWM.csv",  sep=',', encoding='utf-8', low_memory=False)
  ...
  df_selected_LICSNO = df_selected_LICSNO[[u'LICSNO', u'qualified','scrapped_150708', u'PSLTAX', u'EGNO', u'BDNO', u'FRAN', u'CARNM',       u'CARMDL', u'CARTYP', u'YYMDL', u'TRCANO', u'CARSFX',       u'SADLR', u'SABRNH', u'SASALR', u'PRODDT', u'LOADT', u'ISADT',       u'SALEDT', u'LSKIDT', u'REDLDT', u'STSCD', u'STS_DLRBRNH', u'STSDT',       u'STR_DLRBRNH', u'STRDT', u'MT_DLRBRNH', u'MTDT', u'WMI', u'VIN',       u'UCDELIVIDT', u'UCDLR', u'UCBRNH', u'UCENSURTYP', u'UCDELIMILES',       u'UCGUABKNM', u'UCOCGUAEXP', u'UCUCGUAEXP', u'UCOCGUAMIL',       u'UCUCGUAMIL', u'UCSALE', u'CARCD', u'EXCD', u'VDS', u'SEDLDT',       u'FENDAT', u'UENDAT', u'CARENDAT', u'CUSTID1', u'FORCEID1', u'CUSTID2', u'FORCEID2']]

  df_selected_LICSNO['LICSNO']  = df_selected_LICSNO['LICSNO'].map(lambda x: x if (isinstance(x, float)or x is None ) else x.strip() )
  df_selected_LICSNO['VIN']  = df_selected_LICSNO['VIN'].map(lambda x: x if (isinstance(x, float)or x is None ) else x.strip() )
  df_selected_LICSNO['EGNO']  = df_selected_LICSNO['EGNO'].map(lambda x: x if (isinstance(x, float)or x is None ) else x.strip() )
  df_selected_LICSNO['BDNO']  = df_selected_LICSNO['BDNO'].map(lambda x: x if (isinstance(x, float)or x is None ) else x.strip() )
  df_selected_LICSNO[u'CUSTID2']  = df_selected_LICSNO['CUSTID2'].map(lambda x: x if (isinstance(x, float)or x is None ) else x.strip() )
  ...
  #改用新的carCNT計算方式
  df_selected_LICSNO['LICSNO'] = df_selected_LICSNO['LICSNO'].map(lambda x: x if (isinstance(x, float)or x is None ) else x.strip() )
  df_Feature_CARCNT['LICSNO'] = df_Feature_CARCNT['LICSNO'].map(lambda x: x if (isinstance(x, float)or x is None ) else x.strip() )
  df_selected_LICSNO = df_selected_LICSNO.merge(df_Feature_CARCNT[[u'LICSNO', u'carCNT11', u'carCNT21', u'carCNT12', u'carCNT22']], how='left', left_on = 'LICSNO', right_on='LICSNO')
  ...
  df_Feature_SRLFOWM_0717['VIN'] = df_Feature_SRLFOWM_0717['VIN'].map(lambda x: x if (isinstance(x, float)or x is None ) else x.strip() )
  ...
  df_Feature_ISrelated['LICSNO'] = df_Feature_ISrelated['LICSNO'].map(lambda x: x if (isinstance(x, float)or x is None ) else x.strip() )
  df_selected_LICSNO['LICSNO'] = df_selected_LICSNO['LICSNO'].map(lambda x: x if (isinstance(x, float)or x is None ) else x.strip() )
  ...
  df_selected_LICSNO.EGNO = df_selected_LICSNO.EGNO.map(lambda x: x if (isinstance(x, float)or x is None ) else x.strip() )
  df_selected_LICSNO = df_selected_LICSNO.merge(df_Feature_LRDT, how='left', left_on = 'VIN', right_on='VIN')  #這邊left_on 改為vin
  ...
  df_Feature_SR_WKCTCD.index = df_Feature_SR_WKCTCD.index.map(lambda x: x if (isinstance(x, int)or x is None ) else x.strip())
  df_selected_LICSNO.VIN = df_selected_LICSNO.VIN.map(lambda x: x if (isinstance(x, int)or x is None ) else x.strip())
  df_selected_LICSNO = df_selected_LICSNO.merge(df_Feature_SR_WKCTCD, how='left', left_on ='VIN', right_on='index')   #這邊left_on 改為vin
  ...
  df_Feature_CSMCFORM_byLICSNO.LICSNO = df_Feature_CSMCFORM_byLICSNO.LICSNO.map(lambda x: x if (isinstance(x, int)or x is None  or isinstance(x, float)) else x.strip())
  df_selected_LICSNO = df_selected_LICSNO.merge(df_Feature_CSMCFORM_byLICSNO, how='left', left_on ='LICSNO', right_on='LICSNO')  
  ...
  df_feature_SRMINVO['VIN']  = df_feature_SRMINVO['VIN'].map(lambda x: x if (isinstance(x, float)or x is None ) else x.strip() )
  ...
  df_feature_SRPPDPMF['VIN']  = df_feature_SRPPDPMF['VIN'].map(lambda x: x if (isinstance(x, float)or x is None ) else x.strip() )
  ...
  df_feature_SRPPDCMF['VIN']  = df_feature_SRPPDCMF['VIN'].map(lambda x: x if (isinstance(x, float)or x is None ) else x.strip() )
  ...
  for index in obj_list:
      df_selected_LICSNO[index] = df_selected_LICSNO[index].map(lambda x: x if (isinstance(x, float)or x is None ) else x.strip())    
  ...
  conn.close()
  df_PSLTAXORDMF_forNewcarRemove = df_PSLTAXORDMF_forNewcarRemove[df_PSLTAXORDMF_forNewcarRemove[u'DELDT']<datetime.datetime(2010,1,1,0,0,0,0)]
  df_PSLTAXORDMF_forNewcarRemove = df_PSLTAXORDMF_forNewcarRemove[~(df_PSLTAXORDMF_forNewcarRemove['LICSNO'].isnull())]
  ...
  df_LICSNO0717_Features = pd.read_csv(Temp_Path+"df_selected_LICSNO_Features.csv", sep=',', encoding='utf-8', low_memory=False)
  df_LICSNO0717_Features = df_LICSNO0717_Features.merge(df_SRWHMF, how = 'left',on='LICSNO') # 將 LRKM 的資料寫入
  df_LICSNO0717_Features = df_LICSNO0717_Features[df_LICSNO0717_Features['LRKM'] > 5000] # LRKM < 5000的濾除（假設5000以後沒回廠，視為非忠實客戶），濾除284筆(201701)
  del df_SRWHMF
  ...
  conn = pyodbc.connect(DSN="Simba Spark ODBC Driver",autocommit=True, unicode_results=True)
  df_PSLTAXORDMF_for_CARAGE = pd.read_sql(u"""select * from cdp.psltaxordmf where defunctdt < '""" + END_DATE +"""' and length(LSKCUSTID) = 10 and OLICSNO<>LICSNO""",conn)
  conn.close()
  ...
  df_LICSNO0717_Features.loc[df_LICSNO0717_Features['LICSNO'].isin(temp['LICSNO']),'CARAGE'] = df_LICSNO0717_Features.loc[df_LICSNO0717_Features['LICSNO'].isin(temp['LICSNO']),'CARAGE_1']
  ...
  df_LICSNO0717_Features['LICSNO'] = df_LICSNO0717_Features['LICSNO'].map(lambda x: x.strip().upper())
  #刪除汰舊換新新車車輛
  df_LICSNO0717_Features = df_LICSNO0717_Features[~df_LICSNO0717_Features['LICSNO'].isin(df_PSLTAXORDMF_forNewcarRemove['LICSNO'].tolist())]
  Agg_result_list.append(df_LICSNO0717_Features.groupby(['CUSTID1'])['WSLPDT_count','WSLPDT_count_3year','SRMINVO_TOTAMT_sum','SRMINVO_TOTAMT_sum_3year'].mean())
  Agg_result_list.append(df_LICSNO0717_Features.groupby(['CUSTID1'])['FENDAT_count','FINDT_count','SRMINVO_INVODT_count','CARAGE','SRMINVO_TOTAMT_mean','RTPTDT_count'].mean())
  Agg_result_list.append(df_LICSNO0717_Features.groupby(['CUSTID1'])['WSLPDT_count','WSLPDT_count_3year','CSMCFORM_CASETP_Ccount','CSMCFORM_CASETP_Gcount','CSMCFORM_CASETP_Qcount','CSMCFORM_CASETP_Rcount'].sum())
  result_agg = df_LICSNO0717_Features.groupby(['CUSTID1'])['CARENDAT','FENDAT','FINDT_max','RTPTDT_max','SRMINVO_INVODT_max',                                                         'WSLPDT_max'].agg(['max', 'min'])
  ...
  Agg_result_list[0].columns = ['WSLPDT_count_aggmean','WSLPDT_count_3year_aggmean','SRMINVO_TOTAMT_sum_aggmean','SRMINVO_TOTAMT_sum_3year_aggmean']
  Agg_result_list[1].columns = [u'FENDAT_count_aggmean', u'FINDT_count_aggmean', u'SRMINVO_INVODT_count_aggmean', u'CARAGE_aggmean', u'SRMINVO_TOTAMT_mean_aggmean',u'RTPTDT_count_aggmean']
  Agg_result_list[2].columns = ['WSLPDT_count_aggsum','WSLPDT_count_3year_aggsum','CSMCFORM_CASETP_Ccount_aggsum','CSMCFORM_CASETP_Gcount_aggsum','CSMCFORM_CASETP_Qcount_aggsum','CSMCFORM_CASETP_Rcount_aggsum']
  ...
  df_LICSNO0717_Features.loc[df_LICSNO0717_Features[u'WSLPDT_min'].isnull(), u'WSLPDT_min'] =df_LICSNO0717_Features.loc[df_LICSNO0717_Features[u'WSLPDT_min'].isnull(), u'CARAGE']*365
  df_LICSNO0717_Features.loc[df_LICSNO0717_Features[u'WSLPDT_max'].isnull(), u'WSLPDT_max'] =df_LICSNO0717_Features.loc[df_LICSNO0717_Features[u'WSLPDT_max'].isnull(), u'CARAGE']*365
  df_LICSNO0717_Features[u'WSLPDT_range']=df_LICSNO0717_Features[u'WSLPDT_max']-df_LICSNO0717_Features[u'WSLPDT_min']

  df_LICSNO0717_Features.loc[df_LICSNO0717_Features[u'RTPTDT_min'].isnull(), u'RTPTDT_min'] =df_LICSNO0717_Features.loc[df_LICSNO0717_Features[u'RTPTDT_min'].isnull(), u'CARAGE']*365
  df_LICSNO0717_Features.loc[df_LICSNO0717_Features[u'RTPTDT_max'].isnull(), u'RTPTDT_max'] =df_LICSNO0717_Features.loc[df_LICSNO0717_Features[u'RTPTDT_max'].isnull(), u'CARAGE']*365
  df_LICSNO0717_Features[u'RTPTDT_range']=df_LICSNO0717_Features[u'RTPTDT_max']-df_LICSNO0717_Features[u'RTPTDT_min']

  df_LICSNO0717_Features.loc[df_LICSNO0717_Features[u'STRWKDT_min'].isnull(), u'STRWKDT_min'] =df_LICSNO0717_Features.loc[df_LICSNO0717_Features[u'STRWKDT_min'].isnull(), u'CARAGE']*365
  df_LICSNO0717_Features.loc[df_LICSNO0717_Features[u'STRWKDT_max'].isnull(), u'STRWKDT_max'] =df_LICSNO0717_Features.loc[df_LICSNO0717_Features[u'STRWKDT_max'].isnull(), u'CARAGE']*365
  df_LICSNO0717_Features[u'STRWKDT_range']=df_LICSNO0717_Features[u'STRWKDT_max']-df_LICSNO0717_Features[u'STRWKDT_min']

  df_LICSNO0717_Features.loc[df_LICSNO0717_Features[u'FINDT_min'].isnull(), u'FINDT_min'] =df_LICSNO0717_Features.loc[df_LICSNO0717_Features[u'FINDT_min'].isnull(), u'CARAGE']*365
  df_LICSNO0717_Features.loc[df_LICSNO0717_Features[u'FINDT_max'].isnull(), u'FINDT_max'] =df_LICSNO0717_Features.loc[df_LICSNO0717_Features[u'FINDT_max'].isnull(), u'CARAGE']*365
  ...
  df_LICSNO0717_Features.loc[df_LICSNO0717_Features[u'AGVCARDT_min'].isnull(), u'AGVCARDT_min'] =df_LICSNO0717_Features.loc[df_LICSNO0717_Features[u'AGVCARDT_min'].isnull(), u'CARAGE']*365
  df_LICSNO0717_Features.loc[df_LICSNO0717_Features[u'AGVCARDT_max'].isnull(), u'AGVCARDT_max'] =df_LICSNO0717_Features.loc[df_LICSNO0717_Features[u'AGVCARDT_max'].isnull(), u'CARAGE']*365
  df_LICSNO0717_Features[u'AGVCARDT_range']=df_LICSNO0717_Features[u'AGVCARDT_max']-df_LICSNO0717_Features[u'AGVCARDT_min']
  ...
  df_LICSNO0717_Features.loc[df_LICSNO0717_Features[u'SRMINVO_INVODT_min'].isnull(), u'SRMINVO_INVODT_min'] =df_LICSNO0717_Features.loc[df_LICSNO0717_Features[u'SRMINVO_INVODT_min'].isnull(), u'CARAGE']*365
  df_LICSNO0717_Features.loc[df_LICSNO0717_Features[u'SRMINVO_INVODT_max'].isnull(), u'SRMINVO_INVODT_max'] =df_LICSNO0717_Features.loc[df_LICSNO0717_Features[u'SRMINVO_INVODT_max'].isnull(), u'CARAGE']*365
  df_LICSNO0717_Features[u'SRMINVO_INVODT_range'] = df_LICSNO0717_Features[u'SRMINVO_INVODT_max'] - df_LICSNO0717_Features[u'SRMINVO_INVODT_min']
  ...
  'WSLPDT_max_min_agg','FENDAT_max_agg','FENDAT_min_agg','CARENDAT_max_agg','CARENDAT_min_agg']:
      df_LICSNO0717_Features.loc[df_LICSNO0717_Features[item].isnull(), item]     =df_LICSNO0717_Features.loc[df_LICSNO0717_Features[item].isnull(), u'CARAGE']*365   
  ...
  df_LICSNO0717_Features[u'LRKM_modified'] = (df_LICSNO0717_Features[u'CARAGE']*365.0)*             (df_LICSNO0717_Features[u'LRKM'] / ( (df_LICSNO0717_Features[u'CARAGE']*365.0) -                              df_LICSNO0717_Features[u'WSLPDT_max']))
  ...
  for index1 in range(0, len(select_XY_list)):
      for index2 in range(0, index1):
          if( (df_LICSNO0717_Features[select_XY_list[index1]].dtypes!='object')& (df_LICSNO0717_Features[select_XY_list[index2]].dtypes!='object')):
              if math.fabs(df_LICSNO0717_Features[[select_XY_list[index1], select_XY_list[index2]]].corr().iloc[0,1]) > 0.95:
  ...
  feature_number = 0
  if ((df_feature_desc[df_feature_desc['columns']==X_feature_list[0]]['dtypes'].iloc[0]=='int64')|    (df_feature_desc[df_feature_desc['columns']==X_feature_list[0]]['dtypes'].iloc[0]=='float64')):
      df_LICSNO0717_Features_D  = pd.concat([df_LICSNO0717_Features_D, df_LICSNO0717_Features[X_feature_list[0]] ], axis=1 )
  else:
      df_LICSNO0717_Features_D  = pd.concat([df_LICSNO0717_Features_D, pd.get_dummies(df_LICSNO0717_Features[X_feature_list[0]], prefix=X_feature_list[0])], axis=1 )
  feature_number+=1
    
  for index in range(1, len(X_feature_list)):
      feature_number+=1
      if ((df_feature_desc[df_feature_desc['columns']==X_feature_list[index]]['dtypes'].iloc[0]=='int64')|        (df_feature_desc[df_feature_desc['columns']==X_feature_list[index]]['dtypes'].iloc[0]=='float64')):
          df_LICSNO0717_Features_D  = pd.concat([df_LICSNO0717_Features_D, df_LICSNO0717_Features[X_feature_list[index]] ], axis=1 )
      else:
          df_LICSNO0717_Features_D  = pd.concat([df_LICSNO0717_Features_D, pd.get_dummies(df_LICSNO0717_Features[X_feature_list[index]], prefix=X_feature_list[index])], axis=1 )
  write_Log(Log_File,"ok\n")

  for index in [u'CARNM_-99999', u'SRLFOWM_APPFLG_NAcount',u'TRCANO_NaN',u'SABRNH_na',u'CARCD_NaN', 'SRMINVO_DLRCD_maxoccu_NaN']:
      try:
  ...
  forest = RandomForestClassifier(min_samples_split=34, n_estimators=1500, criterion='gini', min_samples_leaf=16, n_jobs=-1, random_state=12345)
  forest.fit(X,y)
  ...
  gs = GridSearchCV(estimator=rf, param_grid=param_grid, scoring='f1',                   cv=StratifiedKFold(5).split(Xtr,ytr), n_jobs=-1)
  ...
  df_LICSNO_CUSTID_MOBILE['MOBILE'] = df_LICSNO_CUSTID_MOBILE['MOBILE'].map(lambda x: '' if pd.isnull(x) else re.sub('[^0-9]+', '', x))
  ...
  Final_List_with_MOBILE.to_csv(Result_Path+"Final_List_drop_Mobile_duplicates.csv",  sep=',', index=False, encoding='utf-8')
  ...
  df_batch_config = pd.DataFrame(data = batch_config_data, columns=['Model_Date', 'Model_Name', 'Threshold'], index =['1'])
